...
const PUBLIC_QUALIFICATION_QUESTIONS = {
  Wedding: [
    "Which package are you most interested in: Silver, Gold, Platinum, or custom?",
    "How many hours of wedding coverage do you expect?",
    "Do you also need pre-wedding coverage?"
  ],
  Engagement: [
    "Do you want an outdoor session, studio session, or both?",
    "Are you looking for photography, videography, or both?",
    "How many hours of coverage are you planning?"
  ],
  "Baby Shower": [
  ...
  "Gender Reveal": [
    ...
    "Do you want only the reveal covered or the whole event too?",
    "How many hours of coverage are you planning?"
  ],
  "House Warming": [
    "Is this mainly a pooja, a family gathering, or both?",
    "How many guests are you expecting?",
    "Do you want photography only or both photography and videography?"
  ],
  Birthday: [
    "Is it a kids birthday, adult birthday, or a family celebration?",
    "How many guests are expected?",
    "Would you like photography only or both photography and videography?"
  ],
  Other: [
    "What kind of event are you planning?",
    "How many guests are expected?",
    "What kind of coverage do you want from us?"
  ]
};
const PUBLIC_PACKAGE_INFO = {
...
const PUBLIC_PORTFOLIO_MATCHES = [
  {
    title: "Weddings",
...
const publicEnquiryQualifierGrid = document.querySelector("#publicEnquiryQualifierGrid");
const publicAiForm = document.querySelector("#publicAiForm");
const publicAiAnswer = document.querySelector("#publicAiAnswer");
const publicAiAvailability = document.querySelector("#publicAiAvailability");
const publicAiPortfolioMatches = document.querySelector("#publicAiPortfolioMatches");
const publicAiQuestionSuggestions = document.querySelector("#publicAiQuestionSuggestions");
const publicAiSubmit = document.querySelector("#publicAiSubmit");
const publicAiPromptButtons = document.querySelectorAll(".public-ai-prompt");
...
publicAiForm?.addEventListener("submit", handlePublicAiSubmit);
publicAiPromptButtons.forEach((button) => button.addEventListener("click", handlePublicAiPromptClick));
publicEventTypeSelect?.addEventListener("change", () => {
  syncPublicCustomEventField();
  renderPublicQualificationQuestions();
});
...
function initializeForms() {
  ...
  syncPublicCustomEventField();
  renderPublicQualificationQuestions();
  renderPublicAiSuggestions(getPublicQualificationQuestions());
  syncLeadTeamSection();
...
function getPublicSelectedEventType() {
  if (!publicEventTypeSelect) return "Other";
  return publicEventTypeSelect.value || "Other";
}

function getPublicQualificationQuestions(eventType = getPublicSelectedEventType()) {
  return PUBLIC_QUALIFICATION_QUESTIONS[eventType] || PUBLIC_QUALIFICATION_QUESTIONS.Other;
}

function inferPublicAssistantEventType(message = "", eventType = "") {
  if (eventType && eventType !== "Other") return eventType;

  const lowerMessage = message.toLowerCase();
  if (lowerMessage.includes("wedding")) return "Wedding";
  if (lowerMessage.includes("engagement") || lowerMessage.includes("proposal") || lowerMessage.includes("couple")) return "Engagement";
  if (lowerMessage.includes("baby shower")) return "Baby Shower";
  if (lowerMessage.includes("gender reveal") || lowerMessage.includes("reveal")) return "Gender Reveal";
  if (lowerMessage.includes("house warming") || lowerMessage.includes("housewarming") || lowerMessage.includes("griha pravesh")) return "House Warming";
  if (lowerMessage.includes("birthday")) return "Birthday";
  return "Other";
}

function getPublicPortfolioMatches(message = "", eventType = "Other") {
  const haystack = `${message} ${eventType}`.toLowerCase();
  const scoredMatches = PUBLIC_PORTFOLIO_MATCHES
    .map((match) => {
      let score = match.keywords.reduce((total, keyword) => total + (haystack.includes(keyword) ? 1 : 0), 0);
      if (!score && ["Wedding", "Engagement"].includes(eventType) && match.title === "Weddings") {
        score = 1;
      }
      if (!score && ["Baby Shower", "Gender Reveal", "House Warming", "Birthday"].includes(eventType) && match.title === "Event Pictures") {
        score = 1;
      }
      return { score, match };
    })
  ...
  return scoredMatches.length
    ? scoredMatches.slice(0, 2)
    : PUBLIC_PORTFOLIO_MATCHES.map((match) => ({
        title: match.title,
        url: match.url,
        reason: match.reason
      }));
...
function buildOfflinePublicAssistantResponse(payload) {
  const message = String(payload.message || "").trim();
  const eventDate = String(payload.eventDate || "").trim();
  const eventType = inferPublicAssistantEventType(message, String(payload.eventType || "").trim());
  const lowerMessage = message.toLowerCase();

  const asksAvailability = Boolean(eventDate) || ["available", "availability", "free", "open date", "open on", "date"].some((token) => lowerMessage.includes(token));
  const asksPackages = ["package", "packages", "silver", "gold", "platinum", "price", "pricing"].some((token) => lowerMessage.includes(token));
  const asksEvents = ["baby shower", "engagement", "house warming", "housewarming", "birthday", "gender reveal", "do you do", "do you cover"].some((token) => lowerMessage.includes(token));
  const asksPortfolio = ["portfolio", "style", "outdoor", "engagement style", "cinematic", "gallery", "look", "couple"].some((token) => lowerMessage.includes(token));
  ...
  if (asksAvailability) {
    availability = {
      status: "Check in live mode",
      label: eventDate ? `${eventDate} needs a live calendar check` : "Add a preferred date",
      details: "Offline preview can guide the conversation, but date availability needs the local or hosted server running."
    };
    answerParts.push("I can guide the enquiry offline, but live date availability works only when the website server is running.");
  }

  if (asksPackages || !message) {
    answerParts.push(
      `Packages: Silver: ${PUBLIC_PACKAGE_INFO.Silver} Gold: ${PUBLIC_PACKAGE_INFO.Gold} Platinum: ${PUBLIC_PACKAGE_INFO.Platinum}`
    );
  }

  if (asksEvents || !message) {
    answerParts.push("Yes, coverage is available for weddings, engagements, baby showers, gender reveals, house warmings, birthdays, and other family celebrations.");
  }

  const portfolioMatches = getPublicPortfolioMatches(message, eventType);
  if (asksPortfolio || !message) {
    answerParts.push("I matched portfolio galleries below based on the event type or visual style you mentioned.");
  }

  return {
    answer: answerParts.join(" ").trim() || "Ask about packages, event types, dates, or the style you want, and I’ll guide the enquiry.",
    availability,
    portfolioMatches,
    suggestedQuestions: getPublicQualificationQuestions(eventType),
    eventType
  };
}

function renderPublicQualificationQuestions(eventType = getPublicSelectedEventType()) {
  if (!publicEnquiryQualifierGrid) return;
  const questions = getPublicQualificationQuestions(eventType);
  publicEnquiryQualifierGrid.innerHTML = "";

  questions.forEach((question, index) => {
    const label = document.createElement("label");
    label.className = "public-enquiry-qualifier";
    label.dataset.question = question;
    ...
    input.type = "text";
    input.name = `qualification-${index + 1}`;
    input.placeholder = "Your answer";

    label.append(heading, input);
    publicEnquiryQualifierGrid.appendChild(label);
  });

  renderPublicAiSuggestions(questions);
}

function collectPublicQualificationAnswers() {
  if (!publicEnquiryQualifierGrid) return [];

  return Array.from(publicEnquiryQualifierGrid.querySelectorAll(".public-enquiry-qualifier"))
    .map((label) => {
      const input = label.querySelector("input");
      return {
        question: label.dataset.question || "",
...
function setPublicAiAnswer(message) {
  if (!publicAiAnswer) return;
  publicAiAnswer.textContent = message || "Ask something about date, package, event type, or style and I’ll guide you.";
}

function renderPublicAiAvailability(availability) {
  if (!publicAiAvailability) return;
  publicAiAvailability.innerHTML = "";

  if (!availability) {
    publicAiAvailability.textContent = "Add a date and I’ll check whether it looks open.";
    publicAiAvailability.className = "public-ai-placeholder";
    return;
  }

  publicAiAvailability.className = "";
  const badge = document.createElement("div");
  badge.className = `public-ai-availability-status ${availability.status === "Booked" ? "is-booked" : "is-open"}`;
  badge.textContent = availability.status;

  const title = document.createElement("strong");
  title.textContent = availability.label || "";

  const details = document.createElement("div");
  details.className = "public-ai-placeholder";
  details.textContent = availability.details || "";

  publicAiAvailability.append(badge, title, details);
}

function renderPublicAiPortfolioMatches(matches = []) {
  if (!publicAiPortfolioMatches) return;
  publicAiPortfolioMatches.innerHTML = "";

  if (!matches.length) {
    const empty = document.createElement("div");
    empty.className = "public-ai-placeholder";
    empty.textContent = "Tell me your event or preferred style and I’ll match a gallery.";
    publicAiPortfolioMatches.appendChild(empty);
    return;
  }

  matches.forEach((match) => {
    const link = document.createElement("a");
    link.className = "public-ai-match";
    link.href = match.url;
    link.target = "_blank";
    link.rel = "noreferrer";

    const title = document.createElement("strong");
    title.textContent = match.title || "Portfolio";

    const reason = document.createElement("span");
    reason.textContent = match.reason || "";

    link.append(title, reason);
    publicAiPortfolioMatches.appendChild(link);
  });
}

function renderPublicAiSuggestions(questions = []) {
  if (!publicAiQuestionSuggestions) return;
  publicAiQuestionSuggestions.innerHTML = "";

  if (!questions.length) {
    const empty = document.createElement("div");
    empty.textContent = "Once you pick an event type, I’ll suggest the next questions to ask.";
    publicAiQuestionSuggestions.appendChild(empty);
    return;
  }
  ...
  questions.forEach((question) => {
    ...
    publicAiQuestionSuggestions.appendChild(item);
  });
}

function handlePublicAiPromptClick(event) {
  if (!publicAiForm) return;
  const messageField = publicAiForm.elements.message;
  if (!(messageField instanceof HTMLTextAreaElement)) return;
  messageField.value = event.currentTarget.dataset.aiPrompt || "";
  messageField.focus();
}

async function handlePublicAiSubmit(event) {
  event.preventDefault();
  if (!publicAiForm) return;

  const formData = new FormData(publicAiForm);
  const payload = {
  ...
  if (!payload.message && !payload.eventDate && !payload.eventType) {
    setPublicAiAnswer("Ask me about a date, package, event type, or portfolio style and I’ll guide you.");
    return;
  }

  publicAiSubmit.disabled = true;
  publicAiSubmit.textContent = "Thinking…";

  try {
    const response = window.location.protocol === "file:"
      ? buildOfflinePublicAssistantResponse(payload)
      : await apiRequest("/api/public/assistant", {
          method: "POST",
          body: JSON.stringify(payload)
        });

    setPublicAiAnswer(response.answer);
    renderPublicAiAvailability(response.availability);
    renderPublicAiPortfolioMatches(response.portfolioMatches || []);
    renderPublicAiSuggestions(response.suggestedQuestions || []);
  } catch (error) {
    const fallback = buildOfflinePublicAssistantResponse(payload);
    setPublicAiAnswer(fallback.answer);
    renderPublicAiAvailability(fallback.availability);
    renderPublicAiPortfolioMatches(fallback.portfolioMatches || []);
    renderPublicAiSuggestions(fallback.suggestedQuestions || []);
    console.error(error);
  } finally {
    publicAiSubmit.disabled = false;
    publicAiSubmit.textContent = "Ask Assistant";
  }
}
...
async function handlePublicEnquirySubmit(event) {
  ...
  const payload = {
    ...
    qualificationAnswers: collectPublicQualificationAnswers()
  };
...
function renderCalendarView() {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    if (dayItems.length) {
      ...
      // Store event data on the cell for the global tooltip
      cell._calEvents = dayItems;

      cell.addEventListener("mouseenter", (e) => {
        const tip = document.getElementById("calGlobalTooltip");
        if (!tip) return;
        tip.innerHTML = cell._calEvents.map(item =>
          `<div class="cal-tip-row">&#9670; ${escapeHtml(item.title)}</div>`
        ).join("");
        const rect = cell.getBoundingClientRect();
        tip.style.display = "block";
        // Position: prefer right of cell, flip left if near edge
...
function renderTeamDueBreakdown() {
          ...
          ${member.events.map((event) => `
            <div class="overview-team-event" data-type="${event.type}" data-lead-id="${event.leadId || ""}" data-plan-id="${event.planId || ""}" data-event-id="${event.eventId || ""}" data-member-id="${event.memberId || ""}">
              ...
              <div class="team-pay-controls">
                ...
                <button type="button" class="team-pay-full-btn primary-button">Mark Paid ✓</button>
              </div>
  ...
  teamDueBreakdown.querySelectorAll(".overview-team-event").forEach((row) => {
    const { type, leadId, planId, eventId, memberId } = row.dataset;
    const input = row.querySelector(".team-pay-input");
    const partialBtn = row.querySelector(".team-pay-partial-btn");
    const fullBtn = row.querySelector(".team-pay-full-btn");

    const applyPayment = (paymentStatus, paidAmount) => {
      if (type === "lead") {
    ...
    fullBtn.addEventListener("click", () => applyPayment("Completed", null));
